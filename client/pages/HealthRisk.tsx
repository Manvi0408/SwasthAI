          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <AnimatePresence mode="wait">
                {!result ? (
                  ...
                  >
                    <h2 className="text-sm font-semibold text-foreground mb-5 flex items-center gap-2 border-b border-border pb-3">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                          <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Age (years)</label>
                          <input 
                            type="number" 
                            min={1} 
                            max={120} 
                            ...
                            className="w-full bg-muted border border-border rounded-lg px-4 py-2.5 outline-none text-xs font-semibold text-foreground focus:border-border/80 focus:ring-1 focus:ring-ring"
                            required
                          />
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Blood Pressure (Systolic/Diastolic)</label>
                          <input 
                            type="text" 
                        ...
                        <div>
                          <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Activity Level</label>
                          <select 
                            value={activityLevel} 
                            onChange={(e) => setActivityLevel(e.target.value)}
                            className="w-full bg-muted border border-border rounded-lg px-4 py-2.5 outline-none text-xs font-semibold text-foreground focus:border-border/80 focus:ring-1 focus:ring-ring"
                          >
                            <option value="Sedentary" className="bg-card text-foreground">Sedentary (No Exercise)</option>
                            <option value="Light" className="bg-card text-foreground">Lightly Active (1-2 days/week)</option>
                            <option value="Moderate" className="bg-card text-foreground">Moderately Active (3-5 days/week)</option>
                            <option value="Active" className="bg-card text-foreground">Very Active (Daily Heavy)</option>
                          </select>
                        </div>
                      </div>

                      <div>
                        <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Family History (Select primary indicator)</label>
                        <select 
                          value={familyHistory} 
                          onChange={(e) => setFamilyHistory(e.target.value)}
                          className="w-full bg-muted border border-border rounded-lg px-4 py-2.5 outline-none text-xs font-semibold text-foreground focus:border-border/80 focus:ring-1 focus:ring-ring"
                        >
                          <option value="None" className="bg-card text-foreground">No family history of cardiovascular/endocrine disease</option>
                          <option value="Diabetes" className="bg-card text-foreground">Family history of Type 1/2 Diabetes</option>
                          <option value="Heart Disease" className="bg-card text-foreground">Family history of Coronary Heart Disease</option>
                          <option value="Stroke" className="bg-card text-foreground">Family history of Stroke/Hypertension</option>
                          <option value="Multiple" className="bg-card text-foreground">Multiple family history profiles</option>
                        </select>
                      </div>

                      <div className="p-5 bg-muted/65 rounded-lg border border-border space-y-3.5">
                        <span className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider block">Lifestyle Risks Check</span>
                        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 text-xs font-semibold text-muted-foreground">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input 
                              type="checkbox" 
                              checked={diabetes} 
                              onChange={(e) => setDiabetes(e.target.checked)}
                              className="rounded border-border bg-card text-foreground focus:ring-ring w-4 h-4"
                            />
                  ...
                  >
                    ...
                    <div className="bg-card border border-border rounded-lg p-6 shadow-2xl relative overflow-hidden">
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b border-border">
                        <div>
                          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Cumulative Risk Index</span>
                        ...
                        <div className="text-left sm:text-right text-xs text-muted-foreground font-medium">
                          <div>BMI calculated: <span className="font-bold text-foreground">{result.bmi}</span></div>
                          <div className="mt-1">Blood Pressure: <span className="font-bold text-foreground">{bloodPressure}</span></div>
                        </div>
                      </div>
                      ...
                      <div className="h-[210px] w-full mt-6">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={barChartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                            <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#888888', fontWeight: 'bold' }} />
                            <YAxis domain={[0, 100]} tick={{ fontSize: 10, fill: '#888888' }} />
                            <Tooltip 
                              contentStyle={{ background: 'var(--card)', borderColor: 'var(--border)', borderRadius: '6px', fontSize: '11px', color: 'var(--foreground)' }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-card border border-border rounded-lg p-5 shadow-2xl space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className="px-4 py-2 border border-border bg-card hover:bg-muted rounded-lg font-semibold text-xs text-foreground flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-foreground hover:bg-foreground/90 text-background rounded-lg font-semibold text-xs shadow-sm flex items-center gap-1.5 transition-all cursor-pointer"
                      >
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-card border border-border rounded-lg p-6 shadow-2xl text-xs leading-relaxed space-y-4">
                ...
                <div className="space-y-3.5 border-l border-border pl-3.5">
                  <div>
                    <span className="font-semibold text-foreground block mb-0.5">Diabetes Risk:</span>
                    <span className="text-muted-foreground">Evaluated by matching Age weight metrics (BMI), daily movement, and familial genetics.</span>
                  </div>
                  <div>
                    <span className="font-semibold text-foreground block mb-0.5">Heart Disease Risk:</span>
                    <span className="text-muted-foreground">Driven by arterial tension (systolic/diastolic blood pressure), active tobacco smoking status, and blood sugar histories.</span>
                  </div>
                  <div>
                    <span className="font-semibold text-foreground block mb-0.5">Stroke Risk:</span>
                    <span className="text-muted-foreground">Correlated with acute hypertension markers, high alcohol volumes, and aged clinical indicators.</span>
                  </div>
                </div>
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 dark:bg-amber-950/20 dark:border-amber-900/60 rounded-md text-[11px] text-amber-600 dark:text-amber-400 leading-relaxed">
              ...
              <div className="bg-card border border-border rounded-lg p-6 shadow-2xl">
                ...
                <div className="space-y-3.5 max-h-[290px] overflow-y-auto pr-1">
                  {history.length === 0 ? (
                    <p className="text-xs text-muted-foreground text-center py-6">No assessments completed yet.</p>
                  ) : (
                    history.slice(0, 4).map((item) => (
                      <div key={item.id} className="p-4 border border-border rounded-lg bg-muted/40 text-xs flex justify-between items-center gap-4 hover:border-border transition-colors">
                        <div>
                          <div className="font-bold text-foreground">Age: {item.age} • BMI: {item.bmi}</div>
          ...
          <AnimatePresence>
            {showHistory && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-50 bg-black/40 dark:bg-black/60 backdrop-blur-[1px] flex items-center justify-center p-4 sm:p-6"
              >
                <motion.div
                  initial={{ scale: 0.98, y: 8 }}
                  animate={{ scale: 1, y: 0 }}
                  exit={{ scale: 0.98, y: 8 }}
                  className="bg-card border border-border rounded-lg p-6 max-w-4xl w-full max-h-[85vh] flex flex-col shadow-2xl"
                >
                  <div className="flex justify-between items-center border-b border-border pb-4 mb-4">
                    ...
                    <button 
                      ...
                      className="px-3 py-1.5 border border-border bg-card hover:bg-muted rounded text-xs font-semibold text-foreground transition-colors cursor-pointer"
                    >
                      Close
                    </button>
                  </div>
                            ...
                            {history.map((h) => (
                              <tr key={h.id} className="hover:bg-muted/40 transition-colors">
                                <td className="py-3 px-2">
                                  ...
                                  <div className="text-[10px] text-muted-foreground mt-0.5">{new Date(h.timestamp).toLocaleTimeString()}</div>
                                </td>
                                <td className="py-3 px-2">
                                  <div>Age: {h.age} • BP: {h.bloodPressure}</div>
                                  <div className="text-[10px] text-muted-foreground mt-0.5">BMI: {h.bmi} • Smoker: {h.smoking ? "Yes" : "No"}</div>
                                </td>
                                <td className="py-3 px-2 font-bold text-emerald-500">{h.diabetesRisk}%</td>
                                <td className="py-3 px-2 font-bold text-red-500">{h.heartDiseaseRisk}%</td>
                                <td className="py-3 px-2 font-bold text-amber-500">{h.strokeRisk}%</td>
                                <td className="py-3 px-2">
